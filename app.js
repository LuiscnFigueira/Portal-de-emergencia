...
(function () {
  ...
  function add(product) {
    if (!product.id || !product.nome) {
      console.warn("Produto incompleto para carrinho:", product);
      return false;
    }
    const cart = getCart();
  ...
  document.addEventListener("click", function (event) {
    ...
    if (ok) {
      ...
      setTimeout(function () {
        ...
        btn.classList.remove("added-cart");
      }, 1000);
    } else {
...
/* LOGIN_AREA_FIX_START */
(function () {
  const USER_KEYS = ["pe_user", "portal_user", "user", "currentUser"];
  const TOKEN_KEYS = ["pe_token", "portal_token", "token", "sessionToken"];
  ...
  function getStoredToken() {
    ...
    const user = safeParse(localStorage.getItem("pe_user"));
  ...
  function getStoredUser() {
    for (const key of USER_KEYS) {
      ...
      if (parsed && (parsed.email || parsed.nome || parsed.name || parsed.id)) return parsed;
    }
    const token = getStoredToken();
    if (token) {
      return {
        nome: localStorage.getItem("pe_nome") || "Utilizador",
        email: localStorage.getItem("pe_email") || "",
        token: token
      };
    }
    return null;
  ...
  function saveLoginSession(data) {
    const user = data.user || data.utilizador || data || {};
    const token = data.token || user.token || getStoredToken() || "";

    const normalized = {
      id: user.id || user.userId || "",
      nome: user.nome || user.name || user.nomeCliente || "Utilizador",
      email: user.email || "",
      token: token
    };

    localStorage.setItem("pe_user", JSON.stringify(normalized));
    localStorage.setItem("portal_user", JSON.stringify(normalized));
    ...
    if (normalized.nome) localStorage.setItem("pe_nome", normalized.nome);
    if (normalized.email) localStorage.setItem("pe_email", normalized.email);

    return normalized;
  }

  function isAreaPage() {
    return /area-utilizador\.html/i.test(location.pathname) || document.querySelector("#user-orders, .user-orders, .area-utilizador");
  }

  function renderAreaIfLogged() {
    ...
    const reserved = Array.from(document.querySelectorAll("h1,h2,h3,p,div,section"))
      .find(function(el){ return /Acesso reservado/i.test(el.textContent || ""); });

    if (reserved) {
      const card = reserved.closest("section, .app-card, .user-card, div") || reserved;
      card.innerHTML =
        '<h2>Olá, ' + (user && (user.nome || user.name) ? (user.nome || user.name) : "Utilizador") + '</h2>' +
        '<p>Sessão iniciada com sucesso.</p>' +
        '<p><strong>Email:</strong> ' + (user && user.email ? user.email : "—") + '</p>' +
        '<div id="user-orders" class="user-orders"><h3>As minhas encomendas</h3><p>A carregar encomendas...</p></div>' +
        '<button type="button" class="login-btn" id="pe-logout-btn">Terminar sessão</button>';
    }

    document.querySelectorAll(".nav-login-link").forEach(function(el){ el.style.display = "none"; });
    document.querySelectorAll(".nav-user-link").forEach(function(el){ el.style.display = ""; });

    const logout = document.getElementById("pe-logout-btn");
    if (logout && !logout.dataset.bound) {
      logout.dataset.bound = "1";
      logout.addEventListener("click", function () {
        USER_KEYS.concat(TOKEN_KEYS, ["pe_nome", "pe_email"]).forEach(function(k){ localStorage.removeItem(k); });
        location.href = "login.html";
      });
    }

    const ordersBox = document.getElementById("user-orders");
    const api = window.PE_API_URL || "";
    if (ordersBox && api && token && !ordersBox.dataset.loading) {
      ordersBox.dataset.loading = "1";
      const url = api + "?action=myOrders&payload=" + encodeURIComponent(JSON.stringify({ token: token }));
      fetch(url)
        .then(function(r){ return r.json(); })
        .then(function(data){
          const orders = data.orders || data.encomendas || [];
          if (!orders.length) {
            ordersBox.innerHTML = "<h3>As minhas encomendas</h3><p>Ainda não existem encomendas associadas a esta conta.</p>";
            return;
          }
          ordersBox.innerHTML = "<h3>As minhas encomendas</h3>" + orders.map(function(order){
            return '<div class="order-card">' +
              '<strong>Encomenda:</strong> ' + (order.id || order.orderId || "—") + '<br>' +
              '<strong>Total:</strong> ' + (order.total || order.valor || "—") + '€<br>' +
              '<strong>Método:</strong> ' + (order.metodoPagamento || "—") + '<br>' +
              '<strong>Data:</strong> ' + (order.data || "—") +
              '</div>';
          }).join("");
        })
        .catch(function(){
          ordersBox.innerHTML = "<h3>As minhas encomendas</h3><p>Não foi possível carregar as encomendas neste momento.</p>";
        });
    }
  }
  ...
  localStorage.setItem = function (key, value) {
    ...
    if (/user|utilizador/i.test(key)) {
      const parsed = safeParse(value);
      if (parsed && (parsed.token || parsed.email || parsed.nome || parsed.name)) {
        if (key !== "pe_user") saveLoginSession(parsed);
      }
    }
    if (/token/i.test(key) && value) {
      originalSetItem("pe_token", value);
      originalSetItem("portal_token", value);
    }
  };
  ...
  document.addEventListener("submit", function (event) {
    ...
    if (!form) return;
    const isLoginForm = /login/i.test(form.id || "") || /login/i.test(form.className || "") || form.querySelector('input[type="password"]');
    if (!isLoginForm) return;
    setTimeout(function () {
      const user = getStoredUser();
      const token = getStoredToken();
      if (user || token) saveLoginSession({ user: user || {}, token: token });
    }, 800);
  }, true);
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const token = getStoredToken();
    if (user || token) {
      saveLoginSession({ user: user || {}, token: token });
      document.querySelectorAll(".nav-login-link").forEach(function(el){ el.style.display = "none"; });
      document.querySelectorAll(".nav-user-link").forEach(function(el){ el.style.display = ""; });
    }
    renderAreaIfLogged();
  ...
  window.peSession = { getUser: getStoredUser, getToken: getStoredToken, save: saveLoginSession, renderArea: renderAreaIfLogged };
})();
/* LOGIN_AREA_FIX_END */
